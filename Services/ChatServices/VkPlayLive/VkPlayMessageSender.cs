...
public partial class VkPlayMessageSender(ConfigService configService) : IAsyncInitializable
{
    ...
    public async Task InitializeAsync()
    {
        ...
        if (string.IsNullOrEmpty(_authSendToken) || string.IsNullOrEmpty(_channelName)) Log.Warning("Не удалось данные для отправки сообщений в vkplaylive.");
    }
    ...
    public async Task PostMessageAsync(string message, int messageId = 0)
    {
        ...
        {
            Log.Error(ex, "Ошибка при отправке POST-запроса в VkPlayLive.");
        }
    }
